...
- Для работы счетом реализовать следующие возможности:
    - выполнить пополнение на счет;
    - выполнить списание со счета;
    - создать новый счет;
    - закрыть счет.

- Для хранения информации о счетах **можно** (не обязательно) использовать Fake-имплементацию репозитория (хранилища) в виде класса-обертки для коллекции List< Account >.
...
Протестировать слой Bll (NUnit и Moq фреймворки).
    ...
    - Для решения хранения информации использовал ORM Entity Framework,
    - В качестве IoC-контейнера использовал библиотеку Ninject,
    - Протестировал слой Bll с помощью Moq и NUnit, за основу были взяты логики расчеты бонусных баллов,
    - Реализовал простой AutoMapper для сглаживания моделей (сгладить границы слоев, отображение границ).
